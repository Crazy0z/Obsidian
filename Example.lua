...
local function HttpGet(url: string): string
	...
	error("[Obsidian] No HTTP method available (game:HttpGet, syn.request, etc.)")
end

local compile = loadstring or load
if not compile then
	error("[Obsidian] loadstring/load is not available in this environment")
end

local function LoadFromURL(path: string)
	local url = repo .. path
	local source = HttpGet(url)
	if not source or #source < 100 then
		error("[Obsidian] HttpGet returned empty or invalid response for: " .. url)
	end

	local chunk, err = compile(source)
	if not chunk then
		error("[Obsidian] Failed to compile " .. path .. ": " .. tostring(err))
	end

	local ok, result = pcall(chunk)
	if not ok then
		error("[Obsidian] Failed to run " .. path .. ": " .. tostring(result))
	end

	return result
end

local Library = LoadFromURL("Library.lua")
local ThemeManager = LoadFromURL("addons/ThemeManager.lua")
local SaveManager = LoadFromURL("addons/SaveManager.lua")
...
local Window = Library:CreateWindow({
	...
	-- The search bar now includes a clear button when text is typed
...
-- Groupboxes are collapsible by default. Set Collapsible = false to keep one fixed open.
local LeftGroupBox = Tabs.Main:AddLeftGroupbox({
	Name = "Groupbox",
	IconName = "boxes",
	Collapsible = true,
})
local FixedGroupBox = Tabs.Main:AddLeftGroupbox({
	Name = "Fixed Groupbox",
	IconName = "lock",
	Collapsible = false,
})
...
FixedGroupBox:AddToggle("FixedToggle", {
	Text = "Fixed groupbox toggle",
	Default = false,
})
...
LeftGroupBox:AddLabel("Double-click any label to copy its text to the clipboard.")
...
DropdownGroupBox:AddDropdown("MySearchableDropdown", {
	Values = { "This", "is", "a", "searchable", "dropdown", "with", "a", "clear", "button" },
	Default = 1, -- number index of the value / string
	Multi = false, -- true / false, allows multiple choices to be selected
	...
	Searchable = true, -- true / false, makes the dropdown searchable; the clear button appears when text is typed
...
-- Draggable mobile controls: useful for phone/tablet layouts.
Library:AddDraggableButton("Mobile Button", function()
	print("Draggable button clicked!")
end)

local MobileMenu, MobileMenuContainer = Library:AddDraggableMenu("Mobile Menu")
MobileMenuContainer.Position = UDim2.fromOffset(0, 35)
MobileMenuContainer.Size = UDim2.new(1, 0, 1, -35)

local MobileMenuLabel = Instance.new("TextLabel")
MobileMenuLabel.BackgroundTransparency = 1
MobileMenuLabel.Size = UDim2.new(1, -14, 0, 40)
MobileMenuLabel.Position = UDim2.fromOffset(7, 7)
MobileMenuLabel.Text = "This menu is draggable and has rounded glow styling."
MobileMenuLabel.TextSize = 13
MobileMenuLabel.TextWrapped = true
MobileMenuLabel.Parent = MobileMenuContainer
