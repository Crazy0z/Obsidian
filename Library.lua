...
local Library = {
    ...
    NotifyTweenInfo = TweenInfo.new(0.4, Enum.EasingStyle.Quint, Enum.EasingDirection.Out),
    ...
    for _, Groupbox in Tab.Groupboxes do
        ...
        if VisibleElements > 0 then
            if Groupbox.Collapsible and Groupbox.Collapsed then
                Groupbox.WasCollapsedBeforeSearch = true
...
local function ResetTab(Tab)
    ...
    for _, Groupbox in Tab.Groupboxes do
        ...
        if Groupbox.WasCollapsedBeforeSearch then
            Groupbox:SetCollapsed(true, true)
            Groupbox.WasCollapsedBeforeSearch = nil
        end
...
local FetchIcons, Icons = pcall(function()
    return (loadstring(
        game:HttpGet("https://raw.githubusercontent.com/Crazy0z/lucide-roblox-direct/main/source.lua")
    ) :: () -> IconModule)()
end)
...
local function AddAccentGradient(Target: GuiObject)
    ...
    local function GetGradientSequence()
        ...
        local Color2 = Color3.fromHSV((H + 0.08) % 1, math.max(0.2, S - 0.05), math.min(1, V + 0.05))
    ...
    Gradient.Rotation = 45 -- Modern diagonal gradient
    
    Library.Registry[Gradient] = {
        Color = GetGradientSequence
    }
    
    return Gradient
end

local SmoothTweenInfo = TweenInfo.new(0.15, Enum.EasingStyle.Quint, Enum.EasingDirection.Out)
local SoftSpringTweenInfo = TweenInfo.new(0.3, Enum.EasingStyle.Back, Enum.EasingDirection.Out, 0.4)

local function AnimateHoverOutline(Stroke: UIStroke, Accent: boolean)
    TweenService:Create(Stroke, SmoothTweenInfo, {
        Color = Accent and Library.Scheme.AccentColor or Library.Scheme.OutlineColor,
    }):Play()
end
...
function Library:AddDraggableButton(Text: string, Func, ExcludeScaling: boolean?, ExcludeDragging: boolean?)
    ...
    local ButtonOutline = Library:AddOutline(Button)

    Button.MouseEnter:Connect(function()
        AnimateHoverOutline(ButtonOutline, true)
        TweenService:Create(ButtonGlow, SmoothTweenInfo, {
            BackgroundTransparency = 0.62,
        }):Play()
    end)

    Button.MouseLeave:Connect(function()
        AnimateHoverOutline(ButtonOutline, false)
        TweenService:Create(ButtonGlow, SmoothTweenInfo, {
            BackgroundTransparency = 0.82,
        }):Play()
    end)
...
function Library:AddDraggableMenu(Name: string)
    ...
    Holder.MouseEnter:Connect(function()
        AnimateHoverOutline(HolderOutline, true)
        TweenService:Create(HolderGlow, SmoothTweenInfo, {
            BackgroundTransparency = 0.58,
        }):Play()
    end)

    Holder.MouseLeave:Connect(function()
        AnimateHoverOutline(HolderOutline, false)
        TweenService:Create(HolderGlow, SmoothTweenInfo, {
            BackgroundTransparency = 0.78,
        }):Play()
    end)
...
do
    ...
    function Funcs:AddLabel(...)
        ...
        local LabelHolder = New("TextButton", {
            Active = true,
            AutoButtonColor = false,
            BackgroundTransparency = 1,
        ...
        LabelHolder.MouseButton1Click:Connect(function()
            local Now = tick()
            if Now - LastLabelClick > 0.35 then
                LastLabelClick = Now
                return
            end
            ...
            if typeof(setclipboard) == "function" then
                local Success, Error = pcall(setclipboard, Label.Text)
                Library:Notify(
                    Success and {
                        Title = "Copied",
                ...
                Library:Notify({
                    ...
                    Description = "This executor does not expose setclipboard.",
                    Time = 3,
                })
            end
        end)
    ...
    function Funcs:AddButton(...)
        ...
        local function InitEvents(Button)
            Button.Base.MouseEnter:Connect(function()
                ...
                Button.Tween = TweenService:Create(Button.Base, SmoothTweenInfo, {
    ...
    function Funcs:AddCheckbox(Idx, Info)
        ...
        local CheckboxGradient = AddAccentGradient(Checkbox)
        CheckboxGradient.Enabled = Toggle.Value

        Button.MouseEnter:Connect(function()
            if Toggle.Disabled then return end
            TweenService:Create(CheckboxStroke, SmoothTweenInfo, {
        ...
        local CheckImage = New("ImageLabel", {
            ...
            AnchorPoint = Vector2.new(0.5, 0.5),
            Position = UDim2.fromScale(0.5, 0.5),
            Size = UDim2.fromScale(0, 0), -- Start at 0 for scale animation
            Parent = Checkbox,
        })
        ...
        function Toggle:Display()
            ...
            TweenService:Create(Label, SmoothTweenInfo, {
                TextTransparency = Toggle.Value and 0 or 0.4,
            }):Play()
            TweenService:Create(CheckImage, SoftSpringTweenInfo, {
            ...
            CheckboxGradient.Enabled = not Toggle.Disabled and Toggle.Value
        end
    ...
    function Funcs:AddToggle(Idx, Info)
        ...
        local ButtonStroke = New("UIStroke", { Color = "OutlineColor", Parent = Button })
        ...
        local SwitchGradient = AddAccentGradient(Switch)
        SwitchGradient.Enabled = Toggle.Value
        ...
        Button.MouseEnter:Connect(function()
            ...
            TweenService:Create(SwitchStroke, SmoothTweenInfo, {
                Color = Library.Scheme.AccentColor
            }):Play()
        end)
        ...
        function Toggle:Display()
            ...
            SwitchGradient.Enabled = not Toggle.Disabled and Toggle.Value or false
            ...
            TweenService:Create(Ball, SoftSpringTweenInfo, {
    ...
    function Funcs:AddInput(Idx, Info)
        ...
        Box.MouseEnter:Connect(function()
            ...
            AnimateHoverOutline(InputStroke, true)
        end)

        Box.MouseLeave:Connect(function()
            if Input.Disabled then return end
            AnimateHoverOutline(InputStroke, false)
        end)
    ...
    function Funcs:AddSlider(Idx, Info)
        ...
        local HolderStroke = New("UIStroke", { Color = "OutlineColor", Parent = Holder })
        ...
        Bar.MouseEnter:Connect(function()
            if Slider.Disabled then return end
            TweenService:Create(SliderStroke, SmoothTweenInfo, {
                Color = Library.Scheme.AccentColor
            }):Play()
        end)
        ...
        local FillGradient = AddAccentGradient(Fill)
        ...
        New("UIStroke", { Color = "DarkColor", Transparency = 0.5, Parent = Knob })
    ...
    function Funcs:AddDropdown(Idx, Info)
        ...
        DisplayContainer.MouseEnter:Connect(function()
            ...
            TweenService:Create(DisplayStroke, SmoothTweenInfo, {
                Color = Library.Scheme.AccentColor
            }):Play()
...
function Library:Notify(...)
    ...
    if Library.NotificationHistoryContainer then
        local HistItem = New("Frame", {
            BackgroundColor3 = "BackgroundColor",
            Size = UDim2.new(1, 0, 0, 44),
            Parent = Library.NotificationHistoryContainer,
        })
        table.insert(Library.Corners, New("UICorner", { CornerRadius = UDim.new(0, Library.CornerRadius), Parent = HistItem }))
        Library:AddOutline(HistItem)
        ...
        New("TextLabel", {
            ...
            Text = (Data.Title or "Notification") .. " [" .. timeStr .. "]",
            TextSize = 14,
            TextColor3 = "FontColor",
            TextXAlignment = Enum.TextXAlignment.Left,
            Parent = HistItem,
        ...
        local children = Library.NotificationHistoryContainer:GetChildren()
        local frames = {}
...
function Library:CreateWindow(WindowInfo)
    ...
    do
        ...
        Library.NotificationHistoryFrame, Library.NotificationHistoryContainer = Library:AddDraggableMenu("Notification History")
        Library.NotificationHistoryFrame.AnchorPoint = Vector2.new(0, 0.5)
        Library.NotificationHistoryFrame.Position = UDim2.new(0, 6, 0.5, 0)
        Library.NotificationHistoryFrame.Visible = false

        Library.QuickActionBar = New("Frame", {
        ...
        table.insert(Library.Corners, New("UICorner", { CornerRadius = UDim.new(0, WindowInfo.CornerRadius), Parent = Library.QuickActionBar }))
        Library:AddOutline(Library.QuickActionBar)
        
        Library.QuickActionContainer = New("Frame", {
            BackgroundTransparency = 1,
            Size = UDim2.fromScale(1, 1),
            Parent = Library.QuickActionBar,
        })
        New("UIListLayout", {
            FillDirection = Enum.FillDirection.Horizontal,
            VerticalAlignment = Enum.VerticalAlignment.Center,
            Padding = UDim.new(0, 8),
            Parent = Library.QuickActionContainer,
        })
        New("UIPadding", {
            PaddingLeft = UDim.new(0, 8),
            PaddingRight = UDim.new(0, 8),
        ...
        function Library:AddQuickAction(Name, IconName, Callback)
            ...
            table.insert(Library.Corners, New("UICorner", { CornerRadius = UDim.new(0, Library.CornerRadius), Parent = ActionButton }))
            Library:AddOutline(ActionButton)

            local Layout = New("UIListLayout", { FillDirection = Enum.FillDirection.Horizontal, VerticalAlignment = Enum.VerticalAlignment.Center, Padding = UDim.new(0, 6), Parent = ActionButton })
            New("UIPadding", { PaddingLeft = UDim.new(0, 6), PaddingRight = UDim.new(0, 6), Parent = ActionButton })

            if IconName then
                local IconInfo = Library:GetCustomIcon(IconName)
                if IconInfo then
                    New("ImageLabel", {
                        BackgroundTransparency = 1,
                        Image = IconInfo.Url,
                        ImageColor3 = IconInfo.Custom and "WhiteColor" or "AccentColor",
        ...
        local SepLine = Library:MakeLine(MainFrame, {
            Position = UDim2.fromOffset(0, 48),
            Size = UDim2.new(1, 0, 0, 1),
        })
        local SepGradient = Instance.new("UIGradient")
        SepGradient.Parent = SepLine
        local function GetSepGradientSequence()
            return ColorSequence.new({
                ColorSequenceKeypoint.new(0, Library.Scheme.OutlineColor),
                ColorSequenceKeypoint.new(0.5, Library.Scheme.AccentColor),
                ColorSequenceKeypoint.new(1, Library.Scheme.OutlineColor)
            })
        end
        SepGradient.Color = GetSepGradientSequence()
        Library.Registry[SepGradient] = {
            Color = GetSepGradientSequence
        }
        ...
        SearchBox:GetPropertyChangedSignal("Visible"):Connect(UpdateSearchClearButton)
        ...
        local ClockLabel = New("TextLabel", { BackgroundTransparency = 1, Text = "00:00:00", TextSize = 13, TextTransparency = 0.5, AutomaticSize = Enum.AutomaticSize.X, Size = UDim2.fromScale(0, 1), Parent = StatusContainer })
        local FPSLabel = New("TextLabel", { BackgroundTransparency = 1, Text = "60 FPS", TextSize = 13, TextTransparency = 0.5, AutomaticSize = Enum.AutomaticSize.X, Size = UDim2.fromScale(0, 1), Parent = StatusContainer })
        local PingLabel = New("TextLabel", { BackgroundTransparency = 1, Text = "0ms", TextSize = 13, TextTransparency = 0.5, AutomaticSize = Enum.AutomaticSize.X, Size = UDim2.fromScale(0, 1), Parent = StatusContainer })
        ...
        task.spawn(function()
            ...
            while task.wait() do
                ...
                if tick() - LastUpdate >= 1 then
                    ...
                    pcall(function() PingLabel.Text = tostring(math.round(Stats.Network.ServerStatsItem["Data Ping"]:GetValue())) .. "ms" end)
                    Frames = 0
                    LastUpdate = tick()
                end
            end
    ...
    function Window:AddTab(...)
        ...
        do
            ...
            local MainTabContent = New("Frame", {
                BackgroundTransparency = 1,
                Size = UDim2.fromScale(1, 1),
                Parent = TabContainer,
            })
        ...
        function Tab:AddSubTab(SubTabName)
            ...
            MainTabContent.Position = UDim2.fromOffset(0, 36)
            MainTabContent.Size = UDim2.new(1, 0, 1, -36)
            ...
            AddAccentGradient(SubTabGlow)

            local SubTabContent = New("Frame", {
                BackgroundTransparency = 1,
                Size = UDim2.fromScale(1, 1),
                Visible = false,
                Parent = MainTabContent,
            ...
            New("UIListLayout", { Padding = UDim.new(0, 2), Parent = SubTabLeft })
            New("UIPadding", { PaddingBottom = UDim.new(0, 2), PaddingLeft = UDim.new(0, 2), PaddingRight = UDim.new(0, 2), PaddingTop = UDim.new(0, 2), Parent = SubTabLeft })
            ...
            New("UIListLayout", { Padding = UDim.new(0, 2), Parent = SubTabRight })
            New("UIPadding", { PaddingBottom = UDim.new(0, 2), PaddingLeft = UDim.new(0, 2), PaddingRight = UDim.new(0, 2), PaddingTop = UDim.new(0, 2), Parent = SubTabRight })
            ...
            function SubTab:Show()
                if Tab.ActiveSubTab then Tab.ActiveSubTab:Hide() end
                SubTabContent.Visible = true
                TweenService:Create(SubTabButton, SmoothTweenInfo, { TextTransparency = 0 }):Play()
                SubTabGlow.Visible = true
                Tab.ActiveSubTab = SubTab
            end

            function SubTab:Hide()
                SubTabContent.Visible = false
                TweenService:Create(SubTabButton, SmoothTweenInfo, { TextTransparency = 0.5 }):Play()
                SubTabGlow.Visible = false
            end

            SubTabButton.MouseButton1Click:Connect(function() SubTab:Show() end)
        ...
        function Tab:AddGroupbox(Info)
            Info = typeof(Info) == "string" and { Name = Info } or Info or {}
            Info.Name = Info.Name or "Groupbox"
            Info.Collapsible = Info.Collapsible ~= false
            Info.Collapsed = Info.Collapsed == true
            local BoxHolder = New("Frame", { AutomaticSize = Enum.AutomaticSize.Y, BackgroundTransparency = 1, Size = UDim2.fromScale(1, 0), Parent = Info.Side == 1 and self.Sides[1] or self.Sides[2] })
            ...
            local GroupboxList
            do
                GroupboxHolder = New("Frame", { BackgroundTransparency = 1, ClipsDescendants = true, Size = UDim2.fromScale(1, 0), Parent = BoxHolder })
                GroupboxHeader = New("TextButton", { Active = true, AutoButtonColor = false, BackgroundTransparency = 1, Size = UDim2.new(1, 0, 0, 34), Text = "", Parent = GroupboxHolder })

                local BoxIcon = Library:GetCustomIcon(Info.IconName)
                if BoxIcon then
                    New("ImageLabel", { Image = BoxIcon.Url, ImageColor3 = BoxIcon.Custom and "WhiteColor" or "AccentColor", ImageRectOffset = BoxIcon.ImageRectOffset, ImageRectSize = BoxIcon.ImageRectSize, Position = UDim2.fromOffset(6, 6), Size = UDim2.fromOffset(22, 22), Parent = GroupboxHeader })
                end
            ...
            function Groupbox:SetCollapsed(Collapsed: boolean, Instant: boolean?)
                ...
                ChevronLabel.Rotation = Collapsed and -90 or 0
                if Instant then Groupbox:Resize(); Tab:Resize(); return end
                StopTween(Groupbox.ResizeTween)
                local TargetHeight = Groupbox.Collapsed and 34 or GetExpandedHeight()
                Groupbox.ResizeTween = TweenService:Create(GroupboxHolder, SmoothTweenInfo, { Size = UDim2.new(1, 0, 0, TargetHeight) })
                Groupbox.ResizeTween.Completed:Connect(function() Groupbox:Resize(); Tab:Resize() end)
                Groupbox.ResizeTween:Play()
            end
            function Groupbox:Resize()
                ...
                GroupboxHolder.Size = UDim2.new(1, 0, 0, GetExpandedHeight())
            end
            GroupboxHolder.MouseEnter:Connect(function() TweenService:Create(ChevronLabel, SmoothTweenInfo, { TextTransparency = 0 }):Play() end)
            GroupboxHolder.MouseLeave:Connect(function() TweenService:Create(ChevronLabel, SmoothTweenInfo, { TextTransparency = 0.35 }):Play() end)
            GroupboxHeader.MouseButton1Click:Connect(function() Groupbox:SetCollapsed(not Groupbox.Collapsed) end)
        ...
        function Tab:AddTabbox(Info)
            ...
            function Tabbox:UpdateCorners() for _, Tab in Tabbox.Tabs do Tab:UpdateCorners() end end
            function Tabbox:AddTab(Name, IconName)
                ...
                table.insert(Library.Corners, New("UICorner", { CornerRadius = UDim.new(0, WindowInfo.CornerRadius), Parent = Button }))
                local BottomCover = New("Frame", { Name = "BottomCover", BackgroundColor3 = "MainColor", BorderSizePixel = 0, Position = UDim2.new(0, 0, 1, -WindowInfo.CornerRadius), Size = UDim2.new(1, 0, 0, WindowInfo.CornerRadius), Parent = Button })
                local LeftCover = New("Frame", { Name = "LeftCover", BackgroundColor3 = "MainColor", BorderSizePixel = 0, Position = UDim2.new(0, 0, 0, 0), Size = UDim2.new(0, WindowInfo.CornerRadius, 1, 0), Visible = false, Parent = Button })
                local RightCover = New("Frame", { Name = "RightCover", AnchorPoint = Vector2.new(1, 0), BackgroundColor3 = "MainColor", BorderSizePixel = 0, Position = UDim2.new(1, 0, 0, 0), Size = UDim2.new(0, WindowInfo.CornerRadius, 1, 0), Visible = false, Parent = Button })
                ...
                New("UIListLayout", { FillDirection = Enum.FillDirection.Horizontal, HorizontalAlignment = Enum.HorizontalAlignment.Center, VerticalAlignment = Enum.VerticalAlignment.Center, Padding = UDim.new(0, 8), Parent = ButtonContent })
                local ButtonIcon
                if BoxIcon then
                    ButtonIcon = New("ImageLabel", { Image = BoxIcon.Url, ImageColor3 = BoxIcon.Custom and "WhiteColor" or "AccentColor", ImageRectOffset = BoxIcon.ImageRectOffset, ImageRectSize = BoxIcon.ImageRectSize, ImageTransparency = 0.5, Size = UDim2.fromScale(1, 1), SizeConstraint = Enum.SizeConstraint.RelativeYY, Parent = ButtonContent })
                end
                local ButtonLabel = New("TextLabel", { BackgroundTransparency = 1, Size = UDim2.fromOffset(0, 16), Text = Name, TextSize = 14, TextTransparency = 0.5, TextXAlignment = Enum.TextXAlignment.Center, Parent = ButtonContent })
                local Line = New("Frame", { BackgroundColor3 = "AccentColor", BorderSizePixel = 0, Position = UDim2.fromOffset(0, 33), Size = UDim2.new(1, 0, 0, 1), Parent = Button })
                AddAccentGradient(Line)
                local Container = New("Frame", { BackgroundTransparency = 1, Position = UDim2.fromOffset(0, 35), Size = UDim2.new(1, 0, 1, -35), Parent = TabboxHolder })
                ...
                local Tab = { ButtonHolder = Button, Container = Container, ButtonCovers = { BottomCover = BottomCover, LeftCover = LeftCover, RightCover = RightCover }, Tab = Tab, Elements = {}, DependencyBoxes = {} }
                function Tab:Show()
                    if Tabbox.ActiveTab then Tabbox.ActiveTab:Hide() end
                    Button.BackgroundTransparency = 0.48
                    BottomCover.BackgroundTransparency = 0.48
                    LeftCover.BackgroundTransparency = 0.48
                    RightCover.BackgroundTransparency = 0.48
        ...
        function Tab:Hover(Hovering)
            ...
            TweenService:Create(TabLabel, SmoothTweenInfo, { TextTransparency = Hovering and 0.25 or 0.5 }):Play()
            if TabIcon then TweenService:Create(TabIcon, SmoothTweenInfo, { ImageTransparency = Hovering and 0.25 or 0.5 }):Play() end
            if TabGlow then TabGlow.Visible = Hovering; TweenService:Create(TabGlow, SmoothTweenInfo, { BackgroundTransparency = Hovering and 0.22 or 1 }):Play() end
        end

        function Tab:Show()
            if Library.ActiveTab then Library.ActiveTab:Hide() end
            TweenService:Create(TabButton, SmoothTweenInfo, { BackgroundTransparency = 0.48 }):Play()
            if TabGlow then TabGlow.Visible = true; TweenService:Create(TabGlow, SoftSpringTweenInfo, { BackgroundTransparency = 0.12 }):Play() end
            TweenService:Create(TabLabel, SmoothTweenInfo, { TextTransparency = 0 }):Play()
            if TabIcon then TweenService:Create(TabIcon, SmoothTweenInfo, { ImageTransparency = 0 }):Play() end
        ...
        function Tab:Hide()
            TweenService:Create(TabButton, SmoothTweenInfo, { BackgroundTransparency = 1 }):Play()
            ...
            TweenService:Create(TabLabel, SmoothTweenInfo, { TextTransparency = 0.5 }):Play()
            if TabIcon then TweenService:Create(TabIcon, SmoothTweenInfo, { ImageTransparency = 0.5 }):Play() end
    ...
    function Window:AddKeyTab(...)
        ...
        function Tab:Hover(Hovering)
            ...
            TweenService:Create(TabLabel, SmoothTweenInfo, {
                TextTransparency = Hovering and 0.25 or 0.5,
            }):Play()
            if TabIcon then
                TweenService:Create(TabIcon, SmoothTweenInfo, {
                    ImageTransparency = Hovering and 0.25 or 0.5,
                }):Play()
            end
            if TabGlow then
                TabGlow.Visible = Hovering
                TweenService:Create(TabGlow, SmoothTweenInfo, {
                    BackgroundTransparency = Hovering and 0.22 or 1,
                }):Play()
            end
        end
        ...
        function Tab:Show()
            ...
            TweenService:Create(TabButton, SmoothTweenInfo, {
                BackgroundTransparency = 0.48,
            }):Play()
            if TabGlow then
                TabGlow.Visible = true
                TweenService:Create(TabGlow, SoftSpringTweenInfo, {
                    BackgroundTransparency = 0.12,
                }):Play()
            end
            TweenService:Create(TabLabel, SmoothTweenInfo, {
    ...
    function Window:AddDialog(Idx, Info)
        ...
        TweenService:Create(DialogOverlay, SmoothTweenInfo, {
        ...
        TweenService:Create(DialogScale, SoftSpringTweenInfo, {
            Scale = 1
        }):Play()
        ...
        function Dialog:Dismiss()
            ...
            local CloseTween = TweenService:Create(DialogScale, SmoothTweenInfo, { Scale = 0.9 })
            TweenService:Create(DialogOverlay, SmoothTweenInfo, { BackgroundTransparency = 1 }):Play()
            CloseTween:Play()
            
            task.delay(SmoothTweenInfo.Time, function()
        ...
        function Dialog:AddFooterButton(ButtonIdx, ButtonInfo)
            ...
            local ButtonWrap = {
                ...
                SetDisabled = function(self, Disabled)
                    ...
                    if Disabled then
                        TweenService:Create(TextBtn, SmoothTweenInfo, { BackgroundTransparency = 0.5 }):Play()
                        TweenService:Create(BtnLabel, SmoothTweenInfo, { TextTransparency = 0.5 }):Play()
                    else
                        TweenService:Create(TextBtn, SmoothTweenInfo, { BackgroundTransparency = 0 }):Play()
                        TweenService:Create(BtnLabel, SmoothTweenInfo, { TextTransparency = 0 }):Play()
                    end
                end
            }
            ...
            TextBtn.MouseEnter:Connect(function()
                ...
                TweenService:Create(TextBtn, SmoothTweenInfo, {
            ...
            if WaitTime > 0 then
                ...
                task.delay(WaitTime, function()
                    ...
                    if ProgressBar then
                        TweenService:Create(ProgressBar, SmoothTweenInfo, {
                            BackgroundTransparency = 1
                        }):Play()
                    end
                end)
    ...
    if WindowInfo.EnableSidebarResize then
        ...
        SidebarGrabber.MouseEnter:Connect(function()
            TweenService:Create(DividerLine, SmoothTweenInfo, {
    ...
    Library:GiveSignal(UserInputService.InputBegan:Connect(function(Input: InputObject)
        ...
        if Input.KeyCode == Enum.KeyCode.K and UserInputService:IsKeyDown(Enum.KeyCode.LeftControl) then
            if Library.SearchBox and Library.SearchBox.Visible then
                task.defer(function()
                    Library.SearchBox:CaptureFocus()
                    Library.SearchBox.Text = ""
                end)
            end
        end
    end))
...
function Library:CreateLoading(LoadingInfo)
    ...
    function Loading:UpdateLayout()
        ...
        TweenService:Create(MainFrame, SmoothTweenInfo, { Size = UDim2.fromOffset(FinalWidth, FinalHeight) }):Play()
        TweenService:Create(SideBar, SmoothTweenInfo, { Position = UDim2.fromOffset(Loading.ContentWidth, 0), Size = UDim2.new(0, ShowSidebar and Loading.SidebarWidth or 0, 1, 0) }):Play()
        TweenService:Create(Container, SmoothTweenInfo, { Size = UDim2.new(0, ShowSidebar and Loading.ContentWidth or Loading.WindowWidth, 1, 0) }):Play()
    ...
    function Loading:SetCurrentStep(Step)
        ...
        TweenService:Create(SliderFill, SmoothTweenInfo, { Size = UDim2.fromScale(Progress, 1) }):Play()
